...
export async function POST(request: Request) {
  try {
    ...
    if (!files.length) return Response.json({ error: "No image files supplied." }, { status: 400 });
    ...
    for (const file of files) {
      if (!allowed.has(file.type) || file.size > 8 * 1024 * 1024) return Response.json({ error: "Use JPG, PNG or WebP images up to 8 MB each." }, { status: 400 });
    ...
    return Response.json({ error: "Unable to process the upload." }, { status: 500 });
  }
}
