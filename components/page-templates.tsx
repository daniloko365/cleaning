  ...
  return (
    <SiteShell>
      <PageHero eyebrow={eyebrow} title={title} dek={dek} image={image}>
        <div className="page-hero__actions"><Link className="button button--acid" href="/get-quote">Get exact price ↗</Link><Link className="text-link text-link--light" href="/contact">Ask a question →</Link></div>
      </PageHero>
      <section className="section section--ivory"><div className="shell content-layout"><article className="content-prose">{intro && <p className="lede">{intro}</p>}{sections.map((section) => <section key={section.title}><h2>{section.title}</h2><p>{section.body}</p>{section.bullets && <ul>{section.bullets.map((item) => <li key={item}>{item}</li>)}</ul>}</section>)}</article><aside className="content-aside"><p className="eyebrow">Useful next step</p><h3>Start with the item, not a sales call.</h3><p>Choose the textile, add photos and see the current price logic before scheduling.</p><Link className="button button--ink" href="/get-quote">Build my price ↗</Link></aside></div></section>
      {cta && <CTA />}
    </SiteShell>
  );
}
...
export function ServicePage({ slug }: { slug: string }) {
  ...
  const serviceSchema = { "@context": "https://schema.org", "@type": "Service", name: service.name, description: service.problem, areaServed: { "@type": "AdministrativeArea", name: "Orange County, California" }, provider: { "@type": "LocalBusiness", name: "Novaclean", url: "https://novacleanoc.com" }, offers: { "@type": "Offer", price: price.price, priceCurrency: "USD", url: `https://novacleanoc.com/services/${service.slug}`, description: price.scope } };
  return (
    <SiteShell>
...
export function ContactPage() {
  return <SiteShell><PageHero eyebrow="Contact" title="Start with the item. Ask anything else here." dek="Exact-price requests use the photo flow. General, privacy and accessibility questions create a trackable message reference."><div className="page-hero__actions"><Link className="button button--acid" href="/get-quote">Get exact price ↗</Link><Link className="text-link text-link--light" href="/track">Track a request →</Link></div></PageHero><section className="section section--ivory"><div className="shell content-layout"><article className="content-prose"><h2>Send a direct message</h2><p>The verified public phone/text line is added in the consolidated launch check. This form works now and stores the purpose, consent time and a customer reference without inventing contact details.</p><ContactForm /></article><aside className="content-aside"><p className="eyebrow">Choose the right path</p><h3>Price, care or business?</h3><p>Use photo quote for a new item, the NC reference routes for an existing job, and commercial intake for properties or workplaces.</p><Link className="text-link" href="/commercial/request-bid">Commercial brief →</Link></aside></div></section></SiteShell>;
}
